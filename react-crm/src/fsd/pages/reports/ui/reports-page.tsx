...
function buildProductSummary(rows: ReportRow[]): ProductSummaryRow[] {
  const summary = new Map<string, ProductSummaryRow>();

  for (const row of rows) {
    const products = row.products.length
      ? row.products
      : [{ code: "", name: row.productText || "Товар без детализации", quantity: 1, price: row.amount, sum: row.amount, isGift: false }];
  ...
  return Array.from(summary.values()).sort((left, right) => right.amount - left.amount);
}
  ...
  return (
    <section className={styles.page}>
      ...
      >
        ...
        <label>
          ...
          <select value={retailStoreHref} onChange={(event) => setRetailStoreHref(event.target.value)}>
            ...
            {(storesQuery.data ?? []).map((store) => (
              <option key={store.id} value={store.href}>
                {store.name}
              </option>
            ))}
          </select>
        ...
        <label className={styles.searchField}>
          ...
          <input
            ...
            placeholder="Имя, телефон, ИНН, документ или товар"
          />
        </label>
      ...
      <section className={styles.totals}>
        ...
        <article>
          <span>Сумма</span>
          <strong>{formatSom(totals.amount)}</strong>
        </article>
        <article>
          <span>Оплачено</span>
          <strong>{formatSom(totals.paid)}</strong>
        </article>
        <article>
          <span>Не оплачено</span>
          <strong>{formatSom(totals.unpaid)}</strong>
        </article>
        {canViewProfit ? (
          <article>
            <span>Чистая прибыль</span>
            <strong>{formatSom(totals.netProfit)}</strong>
          </article>
        ) : null}
      </section>
        ...
        <section className={styles.clientProducts}>
          <div className={styles.documentsHead}>
            <div>
              ...
              <p>
                Поиск: {appliedClientSearch} · документов: {visibleRows.length} · товаров: {filteredProductSummary.length} · штук: {formatQuantity(productSummaryQuantity)} · сумма: {formatSom(productSummaryAmount)}
              </p>
            </div>
            <BarChart3 size={22} />
          </div>
      ...
      <section className={styles.documents}>
        <div className={styles.documentsHead}>
          ...
          <p>
            {reportQuery.isFetching
              ...
              : `Период: ${formatDate(dateFrom)} - ${formatDate(dateTo)}${customerFilter === "all" ? "" : " · фильтр по типу клиента"}${appliedClientSearch ? ` · поиск: ${appliedClientSearch}` : ""}`}
          </p>
        </div>
          <BarChart3 size={22} />
        </div>
        ...
        <div className={styles.documentList}>
          {visibleRows.map((row) => {
            const products = row.products.length
              ? row.products
              : [{ code: "", name: row.productText || "Товар", quantity: 1, price: row.amount, sum: row.amount, isGift: false }];
            return (
              <article key={row.id} className={styles.documentCard}>
                <header>
                  ...
                  <div>
                    ...
                    {row.customerTypeLabel ? <small>{row.customerTypeLabel}</small> : null}
                  </div>
                  <div>
                    <span>Сотрудник</span>
