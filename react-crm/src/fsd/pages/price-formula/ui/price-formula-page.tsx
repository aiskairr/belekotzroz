    ...
    <section className={styles.page}>
      <header className={styles.header}>
        <div>
          <p>МойСклад</p>
          <h1>Расчет цен</h1>
          <span>Диапазоны закупки, шаблоны групп, приемки и массовое сохранение цен.</span>
        </div>
        <button onClick={() => catalogQuery.refetch()} type="button"><RefreshCw size={17} /> Обновить</button>
      </header>
      ...
      <section className={styles.templateCard}>
        ...
        <button onClick={saveTemplate} type="button"><Save size={16} /> Сохранить</button>
        <button onClick={assignTemplate} type="button"><Copy size={16} /> Скопировать</button>
        <button onClick={deleteTemplate} type="button"><Trash2 size={16} /> Удалить</button>
      </section>

      <section className={styles.tiersGrid}>
        <article>
          <div className={styles.cardTitle}><div><p>Диапазоны</p><h2>Минимальная цена</h2><span>К закупке в сомах добавится сумма из диапазона.</span></div><button onClick={() => setTiers((value) => [...value, { from: "", to: "", amount: "", currency: "kgs" }])} type="button"><Plus size={16} /></button></div>
        ...
        <article>
          <div className={styles.cardTitle}><div><p>Оптовая</p><h2>Оптовая цена</h2><span>Оптовая цена считается в USD по своим диапазонам.</span></div><button onClick={() => setWholesaleTiers((value) => [...value, { from: "", to: "", amount: "", currency: "kgs" }])} type="button"><Plus size={16} /></button></div>
          <div className={styles.tierHead}><span>От USD</span><span>До USD</span><span>Наценка</span><span>Валюта</span><span /></div>
      ...
      <section className={styles.toolbar}>
        ...
        <button onClick={loadSupply} type="button"><PackageSearch size={16} /> Товары из приемки</button>
        <button onClick={selectFiltered} type="button">Выбрать фильтр</button>
        <button onClick={() => calculateSelected()} type="button">Рассчитать выбранные</button>
        <button onClick={save} disabled={!selectedChanges.length || saveMutation.isPending} type="button"><Save size={16} /> Сохранить цены</button>
      </section>

      <section className={styles.summary}>
        <article><span>Загружено</span><strong>{catalogQuery.isLoading ? "..." : formatNumber(products.length)}</strong></article>
      ...
      <section className={styles.table}>
        ...
        <table>
          ...
          <tbody>
            {pageProducts.map((product) => {
              ...
              return (
                <tr key={product.id} className={reason ? styles.skipped : next ? styles.changed : ""}>
        ...
        <footer className={styles.pagination}>
          <button disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))} type="button">Назад</button>
          <span>Страница {Math.min(page, pageCount)} из {pageCount}</span>
          <button disabled={page >= pageCount} onClick={() => setPage((value) => Math.min(pageCount, value + 1))} type="button">Вперед</button>
        </footer>
      </section>
    </section>
  );
