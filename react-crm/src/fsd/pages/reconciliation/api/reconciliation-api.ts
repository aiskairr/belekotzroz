...
export type ReconciliationDetails = {
  ...
  documents: Array<{ id: string; typeLabel: string; name: string; moment: string; webUrl: string; amount: number; paid: number; debt: number; storeName?: string }>;
  payments: Array<{ id: string; name: string; moment: string; webUrl: string; amount: number; description?: string }>;
};
...
export async function getReconciliationDetails(id: string): Promise<ReconciliationDetails> {
  ...
  const mapDoc = (value: unknown) => {
    const row = asRecord(value);
    return { id: asString(row.id), typeLabel: asString(row.typeLabel), name: asString(row.name), moment: asString(row.moment), webUrl: asString(row.webUrl), amount: asNumber(row.amount), paid: asNumber(row.paid), debt: asNumber(row.debt), storeName: asString(row.storeName) };
  };
  const mapPayment = (value: unknown) => {
    const row = asRecord(value);
    return { id: asString(row.id), name: asString(row.name), moment: asString(row.moment), webUrl: asString(row.webUrl), amount: asNumber(row.amount), description: asString(row.description) };
  };
  return {
