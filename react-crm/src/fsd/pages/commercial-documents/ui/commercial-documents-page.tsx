...
type FormState = Omit<
  CommercialPayload,
  "documentType" | "storeHref" | "employeeName" | "branchName" | "items" | "customerHref" | "customerGroups"
> & {
  customerHref: string;
  wholesale: boolean;
};
...
export function CommercialDocumentsPage() {
  ...
  const chooseCustomer = (customer: CommercialCustomer) => {
    setForm({
      ...
      customerCorrAccount: customer.corrAccount || "",
        ...
        <div className={styles.main}>
          <section className={styles.panel}>
            ...
            <div className={styles.formGrid}>
              ...
              <label><span>Корр. счет</span><input value={form.customerCorrAccount} onChange={(event) => setForm({ ...form, customerCorrAccount: event.target.value })} /></label>
          ...
          <section className={styles.panel}>
            ...
            <div className={styles.searchBox}>
              <Search size={17} />
              <input value={productSearch} onChange={(event) => setProductSearch(event.target.value)} placeholder="Выберите строку и найдите товар по названию, коду или штрихкоду" />
              <div className={styles.results}>
                {(productQuery.data ?? []).map((product) => (
                  <button key={product.href || product.id} type="button" onClick={() => chooseProduct(product)}>
                    <strong>{product.name}</strong><span>{product.code || "Без кода"} · Мин: {money(product.minPrice || product.price || 0)} · Опт: {money(product.wholesalePrice || 0)}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className={styles.items}>
              {items.map((item, index) => (
                <article key={item.id} className={item.id === activeItemId ? styles.selectedItem : ""} onClick={() => setActiveItemId(item.id)}>
                  <label><span>Товар {index + 1}</span><input value={item.productName} onChange={(event) => patchItem(item.id, { productName: event.target.value, assortmentHref: "" })} /></label>
